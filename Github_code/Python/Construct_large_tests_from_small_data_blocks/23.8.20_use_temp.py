...
def mul_atoms(mul_x, mul_y, mul_z, r_a, r_b, r_c, r_d, r_e):
    ...
    generated_coords_inner = set()

    # 生成不重复的随机点坐标
    # 形成一组体内的点，故倍数一定要是4倍以上，至少
    num_points_inner = (max_coord_x) * (max_coord_y) * (max_coord_z)
    points_inner_r = []
    while len(points_inner_r) < num_points_inner:
        x = random.randint(min_coord_x, max_coord_x)
        y = random.randint(min_coord_y, max_coord_y)
        z = random.randint(min_coord_z, max_coord_z)

        coord = (x, y, z)
        if coord not in generated_coords_inner:
            generated_coords_inner.add(coord)
            points_inner_r.append(coord)

    points_inner = np.array(points_inner_r)
    print(points_inner)

    # 打乱点的顺序，以确保随机性
    np.random.shuffle(points_inner) #
    ...
    list_all = list(generated_coords)
    #
    # count = 0

    # print(f'有{count}个点位于inner外')
    ...
    # 只能在块体内部进行选择
    ia_num = int(ratio_ia * num_points_inner)
    ib_num = int(ratio_ib * num_points_inner)
    ic_num = int(ratio_ic * num_points_inner)
    id_num = int(ratio_id * num_points_inner)
    ie_num = int(ratio_ie * num_points_inner)
    # ori可以在外部进行选择
    ori_num = int(num_points - (ie_num + id_num + ic_num + ib_num + ia_num))
    ...
    for num in range(1, num_points_inner + 1):
        if num in range(1, ia_num + 1):
            ia_position_alti[num - 1, :] = points_inner[num - 1, :]
        elif num in range(ia_num + 1, ia_num + ib_num + 1):
            ib_position_alti[num - ia_num - 1, :] = points_inner[num - 1, :]
        elif num in range(ia_num + ib_num + 1, ia_num + ib_num + ic_num + 1):
            ic_position_alti[num - ia_num - ib_num - 1, :] = points_inner[num - 1, :]
        elif num in range(ia_num + ib_num + ic_num + 1, ia_num + ib_num + ic_num + id_num + 1):
            id_position_alti[num - ia_num - ib_num - ic_num - 1, :] = points_inner[num - 1, :]
        elif num in range(ia_num + ib_num + ic_num + id_num + 1, ia_num + ib_num + ic_num + id_num + ie_num + 1):
            ie_position_alti[num - ia_num - ib_num - ic_num - id_num - 1, :] = points_inner[num - 1, :]
        else:
            ori_position_alti[num - ia_num - ib_num - ic_num - id_num - ie_num - 1, :] = points_inner[num - 1, :]

    # 内部点位分配完成后，将余下的点，全部分配给ori
    points_rest = []
    count = -1
    for i in range(1, points.shape[0] + 1):
        if list_all[i - 1] not in generated_coords_inner:
            count += 1
            points_rest.append(list_all[i - 1])
            points_rest_np = np.array(points_rest)
            ori_position_alti[num_points_inner + count - ia_num - ib_num - ic_num - id_num - ie_num, :] = points_rest_np[count, :]
            # print("以下这些点并不在所有点的范围之内：", list_all[i - 1])
...
if __name__ == "__main__":
    ...
    for i in range(1, examples_num):
        ...
        x_max = get_number_from_line(f'{file_path_ori}', 3, 1) * mul_x
        x_min = get_number_from_line(f'{file_path_ori}', 3, 0)
        y_max = get_number_from_line(f'{file_path_ori}', 4, 1) * mul_y
        y_min = get_number_from_line(f'{file_path_ori}', 4, 0)
        z_max = get_number_from_line(f'{file_path_ori}', 5, 1) * mul_z
        z_min = get_number_from_line(f'{file_path_ori}', 5, 0)
    ...
    running_time = f'运行时间为：{elapsed_time}s\n \t{float(elapsed_time/60)}min\n'  # 秒

    # 创建一个用于存储实验相关信息的文本
    file_storage = '/home/ps/YMZ/py_tests_mul/ori/ori/info.txt'
    create_txt_file(file_storage)

    # 存储数据内容
    file_data = ''

    num_a = int(r_a*mul_x*(mul_y)*(mul_z))
    num_b = int(r_b * mul_x * (mul_y) * (mul_z))
    num_c = int(r_c * mul_x * (mul_y) * (mul_z))
    num_d = int(r_d * mul_x * (mul_y) * (mul_z))
    num_e = int(r_e * mul_x * (mul_y) * (mul_z))
    num_ori = mul_x * mul_y * mul_z - num_a - num_b - num_c - num_d - num_e

    id_multi = 80*mul_x*mul_y*mul_z
    atoms_num = f'粒子总数为：{id_multi}\n\n'

    line_ia = f'ia缺陷构型的总数为： {num_a}，大约占比为：{float(num_a/id_multi*100)}%\n'
    line_ib = f'ib缺陷构型的总数为： {num_b}，大约占比为：{float(num_b/id_multi*100)}%\n'
    line_ic = f'ic缺陷构型的总数为： {num_c}，大约占比为：{float(num_c/id_multi*100)}%\n'
    line_id = f'id缺陷构型的总数为： {num_d}，大约占比为：{float(num_d/id_multi*100)}%\n'
    line_ie = f'ie缺陷构型的总数为： {num_e}，大约占比为：{float(num_e/id_multi*100)}%\n'
    line_ori = f'ori完美晶格的总数为： {num_ori}\n'
    ...
    file_data += running_time
    add_lines_to_txt_file(file_storage, file_data, -1)